"""Router tests for pdf (T-20): endpoint wiring plus applicant and principal access.

The tests replace the PDF service with a fake.
"""

from __future__ import annotations

from collections.abc import Iterator
from uuid import UUID, uuid4

import pytest
from fastapi import FastAPI
from fastapi.testclient import TestClient

from app.db import get_session
from app.deps import (
    Applicant,
    Principal,
    get_current_applicant,
    get_current_principal,
)
from app.main import create_app
from app.modules.pdf.models import RenderJob
from app.modules.pdf.router import get_pdf_service
from app.modules.pdf.schemas import JobOut
from app.shared.errors import NotFoundError
from tests._support.files_fakes import FakeStorage

APP_ID = uuid4()
JOB_ID = uuid4()


class _FakeSession:
    async def commit(self) -> None: ...

    async def scalar(self, *_a: object, **_k: object) -> None:
        # created_by lookup (#24): no hit, so no creator bonus and the 403 stands.
        return None

    async def execute(self, *_a: object, **_k: object) -> _EmptyResult:
        return _EmptyResult()  # no Gremium memberships (#vote-read)


class _EmptyResult:
    def scalars(self) -> _EmptyResult:
        return self

    def all(self) -> list[object]:
        return []

    def scalar_one_or_none(self) -> None:
        return None


class _FakeService:
    def __init__(self) -> None:
        self.created: list[UUID] = []
        self.job_status = "done"

    async def create_application_job(
        self, application_id: UUID, *, idempotency_key: str | None = None
    ) -> RenderJob:
        self.created.append(application_id)
        job = RenderJob(application_id=application_id, status="pending")
        job.id = JOB_ID
        return job

    async def get_job(self, job_id: UUID) -> RenderJob:
        if str(job_id).startswith("00000000"):
            raise NotFoundError("nope")
        job = RenderJob(application_id=APP_ID, status=self.job_status, storage_key="k")
        job.id = job_id
        return job

    def to_out(self, job: RenderJob, *, storage: object = None) -> JobOut:
        return JobOut(
            id=job.id,
            kind=job.kind or "application_pdf",
            status=job.status,
            applicationId=job.application_id,
            resultUrl=f"/api/jobs/{job.id}/download" if job.status == "done" else None,
        )


@pytest.fixture
def fake_service() -> _FakeService:
    return _FakeService()


@pytest.fixture
def app(fake_service: _FakeService) -> FastAPI:
    application = create_app()
    application.dependency_overrides[get_pdf_service] = lambda: fake_service

    def _session() -> Iterator[_FakeSession]:
        yield _FakeSession()

    application.dependency_overrides[get_session] = _session
    return application


@pytest.fixture
def client(app: FastAPI) -> TestClient:
    return TestClient(app)


def _principal(app: FastAPI, *perms: str) -> None:
    app.dependency_overrides[get_current_principal] = lambda: Principal(
        sub="p", permissions=set(perms)
    )


def _applicant(app: FastAPI, application_id: UUID, scope: str = "view") -> None:
    app.dependency_overrides[get_current_applicant] = lambda: Applicant(
        application_id=str(application_id),
        scope=scope,  # type: ignore[arg-type]
    )


def test_create_pdf_requires_auth_401(client: TestClient) -> None:
    assert client.post(f"/api/applications/{APP_ID}/pdf").status_code == 401


def test_create_pdf_forbidden_without_read(app: FastAPI, client: TestClient) -> None:
    _principal(app, "other.perm")
    assert client.post(f"/api/applications/{APP_ID}/pdf").status_code == 403


def test_create_pdf_accepted_principal(
    app: FastAPI, client: TestClient, fake_service: _FakeService
) -> None:
    _principal(app, "application.read")
    r = client.post(f"/api/applications/{APP_ID}/pdf")
    assert r.status_code == 202
    body = r.json()
    assert body["status"] == "pending"
    assert body["applicationId"] == str(APP_ID)
    assert fake_service.created == [APP_ID]


def test_create_pdf_accepted_applicant(app: FastAPI, client: TestClient) -> None:
    _applicant(app, APP_ID, "view")
    assert client.post(f"/api/applications/{APP_ID}/pdf").status_code == 202


def test_get_job_requires_auth_401(client: TestClient) -> None:
    assert client.get(f"/api/jobs/{JOB_ID}").status_code == 401


def test_get_job_ok_principal(app: FastAPI, client: TestClient) -> None:
    _principal(app, "application.read")
    r = client.get(f"/api/jobs/{JOB_ID}")
    assert r.status_code == 200
    assert r.json()["resultUrl"] == f"/api/jobs/{JOB_ID}/download"


def test_get_job_not_found_404(app: FastAPI, client: TestClient) -> None:
    _principal(app, "application.read")
    r = client.get("/api/jobs/00000000-0000-0000-0000-000000000000")
    assert r.status_code == 404


def test_get_job_cross_tenant_applicant_404(app: FastAPI, client: TestClient) -> None:
    # An applicant of a *different* application gets 404, not an existence oracle.
    _applicant(app, uuid4(), "view")
    r = client.get(f"/api/jobs/{JOB_ID}")
    assert r.status_code == 404


def test_get_job_applicant_of_owning_application_ok(app: FastAPI, client: TestClient) -> None:
    _applicant(app, APP_ID, "view")
    assert client.get(f"/api/jobs/{JOB_ID}").status_code == 200


# --- GET /jobs/{id}/download -------------------------------------------------
#
# The route replaced a presigned MinIO URL. MinIO has no published port, so the browser
# could never resolve the host in that signature. These tests hold the access rules of
# `get_job` and the storage gates.


def _with_storage(app: FastAPI, storage: object | None) -> None:
    app.state.object_storage = storage


def test_download_requires_auth_401(client: TestClient) -> None:
    assert client.get(f"/api/jobs/{JOB_ID}/download").status_code == 401


def test_download_not_found_404(app: FastAPI, client: TestClient) -> None:
    _principal(app, "application.read")
    r = client.get("/api/jobs/00000000-0000-0000-0000-000000000000/download")
    assert r.status_code == 404


def test_download_cross_tenant_applicant_404(app: FastAPI, client: TestClient) -> None:
    # Same rule as the status route: 404, never 403, so there is no existence oracle.
    _applicant(app, uuid4(), "view")
    assert client.get(f"/api/jobs/{JOB_ID}/download").status_code == 404


def test_download_without_object_storage_503(app: FastAPI, client: TestClient) -> None:
    _principal(app, "application.read")
    _with_storage(app, None)
    assert client.get(f"/api/jobs/{JOB_ID}/download").status_code == 503


def test_download_unfinished_job_409(
    app: FastAPI, client: TestClient, fake_service: _FakeService
) -> None:
    _principal(app, "application.read")
    _with_storage(app, FakeStorage())
    fake_service.job_status = "running"
    r = client.get(f"/api/jobs/{JOB_ID}/download")
    assert r.status_code == 409
    assert r.json()["code"] == "render_not_ready"


def test_download_streams_the_pdf_principal(app: FastAPI, client: TestClient) -> None:
    storage = FakeStorage()
    storage.objects["k"] = (b"%PDF-1.7 body", "application/pdf")
    _principal(app, "application.read")
    _with_storage(app, storage)
    r = client.get(f"/api/jobs/{JOB_ID}/download")
    assert r.status_code == 200
    assert r.content == b"%PDF-1.7 body"
    assert r.headers["content-type"] == "application/pdf"
    assert "attachment;" in r.headers["content-disposition"]
    assert r.headers["x-content-type-options"] == "nosniff"
    # The bytes come through the API. Nothing was presigned.
    assert storage.signed == []


def test_download_streams_for_the_owning_applicant(app: FastAPI, client: TestClient) -> None:
    storage = FakeStorage()
    storage.objects["k"] = (b"pdf", "application/pdf")
    _applicant(app, APP_ID, "view")
    _with_storage(app, storage)
    r = client.get(f"/api/jobs/{JOB_ID}/download")
    assert r.status_code == 200
    assert r.content == b"pdf"
