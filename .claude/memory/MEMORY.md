...
one file that holds one durable fact (a convention, a spec, a status note, a gotcha). Read the
linked file before you start related work. A `[[slug]]` marker inside a file points at a
sibling memory here.

> **Note:** this repo deliberately does **not** mirror the internal security-audit findings.
> They live only in the maintainer's private memory. Do not add them here.

## How to work on this repo (conventions & preferences)

- [work-autonomously](work-autonomously.md) — full-auto execution, pause only via the question tool
...
- [no-narrative-comments](no-narrative-comments.md) — one line, only for a non-obvious why; never narrate the fix
