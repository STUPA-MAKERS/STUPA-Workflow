...
- `DELETE /api/protocols/{protocol_id}` — delete a draft, 204. The `protocol_vote_ref` rows cascade, and the stored PDF objects go from MinIO after the commit. A `final` protocol answers 409 `protocol_finalized`, a `rendering` one 409 `protocol_rendering`. Same gate as the PATCH (`authorize_write`).
...
- **Variant and corporate design:** `protocol.cd_variant` snapshots the CD key of the gremium at creation, so a later CD change does not rewrite an old protocol. `_render_pdf` resolves that key with `resolve_cd_variant_by_key` (`be-admin`). On a hit the pytex variant is the fixed `protocol` shape, and the design contributes only logos: `cd_render_config` builds the `config` object (`logos` / `footer_logos`) and `cd.assets` carries the uploaded logo bytes as multipart `assets`. On a miss `protocol_variant_for` is the fallback — `cd_variant` in {`stupa`,`asta`} maps to `protocol-<cd>`, any other value to `None`, and pytex then infers the variant from the `typ: protokoll` frontmatter. An uploaded logo needs MinIO, so a resolve without storage raises `ServiceUnavailableError`.
