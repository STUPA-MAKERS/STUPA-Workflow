...
- Migration 0002 creates the tables on a fresh schema via `Base.metadata.create_all`. Migration 0013 backfills them idempotently (`checkfirst`) on older schemas.
...
- `GET /api/protocols/{protocol_id}/pdf/public` — stream the redacted public variant (404 unless a non-public TOP exists). Same read perms.
...
- **Source of truth for the body is the per-TOP agenda editor** when TOPs exist: `_assemble_from_agenda` stitches each `MeetingAgendaItem.body` (with `demote_headings`) + its non-cancelled `Vote` tally snippets. It falls back to the free-edited `protocol.markdown` only when the meeting has zero agenda items.
- **TOP headings are top-level `#` with NO "TOP n:" prefix** — pytex numbers the sections automatically as "TOP 1", "TOP 2". The builder demotes body headings one level, so pytex does not count them as new TOPs.
- **Public vs internal (dual render):** if any agenda item has `non_public=True`, finalize renders BOTH a full internal PDF and a redacted public PDF. The redacted variant replaces the non-public TOP bodies and votes with a placeholder and keeps the heading and the numbering. Only the **public** variant goes to the mailing list. Otherwise finalize renders one internal PDF and mails it.
