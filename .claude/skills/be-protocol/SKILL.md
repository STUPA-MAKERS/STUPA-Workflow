...
- **RCE hardening (cross-link `be-pdf`):** the body is user-written, so `sanitize_user_markdown` strips the pytex `eval`-escape (`[//]: # "EXPR"`) and `\iffalse{pytex(...)}\fi`, and neutralizes absolute and `..`-traversal image paths. The service also renders this path with `trust_level="untrusted"` (eval locked + sandbox). These are two independent layers. The `trusted` default in `be-pdf` is for server-generated documents; the protocol is the only caller left.
