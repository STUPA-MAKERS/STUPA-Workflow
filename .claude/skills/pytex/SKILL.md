---
name: pytex
description: Internal-only Markdown→PDF/LaTeX render microservice — a thin FastAPI wrapper (app.py) around pytex_api.render_blob_async (pytex-preprocessor 1.1.0 + tectonic). Single POST /render route, trust levels untrusted/sandboxed/trusted, CD variants report/protocol-stupa/protocol-asta/report-makers/plain. Use when working on PDF/protocol rendering, the /render contract, tectonic/cache/Dockerfile, or the backend PytexClient in pytex/ and backend/app/modules/pdf.
---
...
- `pyproject.toml` — pins `pytex-preprocessor==1.1.0` and `python-multipart` (starlette form parsing). It packages only `app.py`. It adds ruff + basedpyright and the pytest `integration` marker.
- `tests/conftest.py` — `RenderRecorder` fake (monkeypatches `render_blob_async`) + `make_result`/`client` fixtures.
- `tests/test_render.py` — contract tests (mocked backend): success shape, forwarding, body limits, enum/error map. `tests/test_render_multipart.py` — the multipart shape: source/config/assets forwarding plus the asset caps. `tests/test_integration.py` — real md→tex per variant (no tectonic) + md→pdf (skipped unless `tectonic` on PATH). `tests/test_health.py` — health probe.
- `README.md` — endpoint/param/env/error tables.

**Domain / data model:** No database, no persistence. Every request is stateless. Each build runs in a per-request temp dir inside the library. The only durable state is the `pytex_cache` volume at `/cache` (tectonic bundle + LaTeX packages). Library types come from `pytex_api`: `BuildRequest` (source bytes, `input_kind`, `output_kind`, `trust`, `variant`, `config`, `assets`, `limits`), `BuildResult` (`output` bytes, `output_kind`, `log`, `warnings`, `duration_s`), `BuildLimits` (`wall_timeout_s`, `cpu_timeout_s`, `max_input_bytes`). Enums: `InputKind` (`md`/`tex`/`py`, where `md`=`InputKind.MARKDOWN`), `OutputKind` (`tex`/`pdf`), `TrustLevel` (`untrusted`/`sandboxed`/`trusted`). Variants are strings: `report`, `protocol-stupa`, `protocol-asta`, `report-makers`, `plain`. When you omit the variant, the library reads it from the YAML frontmatter. Errors: `LimitError`, `TrustError`, `CompileError`, `ApiError`.
...
- `POST /render` also accepts `multipart/form-data` on the same route with the same query params: `source` (the document bytes, field or file part), `config` (a JSON **object** string; it overrides the frontmatter), and the repeated file part `assets` (its **file name** is the asset name). This is the channel that pushes uploaded Corporate-Design logos into a render: name an asset in the `logos` or `footer_logos` config key and pytex 1.1.0 resolves it to that uploaded file. Any other field name → 400.
- `GET /health` — `{"status": "ok"}`, dependency-free. It drives the compose healthcheck.

Error map. The service replaces every absolute path in an error detail with `<path>` via `_PATH_RE`. Empty source, bad enum, malformed multipart, unknown multipart field, or a `config` that is not a JSON object → 400. A bad asset name comes back from `pytex_api.validate_asset_name` as a `TrustError` → 400; the service never checks the name itself. Body > `PYTEX_MAX_BODY_BYTES` → 413 (checked against `Content-Length` before the read, then again after the read). `LimitError` → 413. `TrustError`/`CompileError`/`ApiError` → 400. Anything else → 500 `{"error":"internal render error"}`, which never leaks a stacktrace.
...
- **Two client wire shapes:** `PytexClient.render_pdf(markdown, *, variant, trust_level, config, assets)` sends multipart only when `config` or `assets` is non-empty. Otherwise it sends the raw body exactly as before, so no existing caller changes.
...
- **The asset caps sit under the body cap.** `source` plus every asset must fit in `PYTEX_MAX_BODY_BYTES`. `PYTEX_MAX_ASSETS` (16) caps the count and `PYTEX_MAX_ASSET_BYTES` (2 MiB) caps a single asset. Each one answers 413, the same shape as `LimitError`.
...
- **Title-page patch:** pytex 1.1.0 renders only its hard-wired frontmatter keys. At import, `app.py` appends `Gremium` and `Beschlussfähigkeit` rows to `_protocol_document._SCALAR_ROWS`. This is a wrapper patch, not a fork. Re-check it after every `pytex-preprocessor` bump.
