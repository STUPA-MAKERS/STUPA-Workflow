...
- `cd_logos.py` — corporate-design logo policy: the vendored pytex logo names, the slot/base-variant value sets, the upload allowlist (PNG/JPEG/WebP/**PDF**), the per-logo and per-variant caps, `sniff_cd_logo`, `asset_file_name`.
- `cd_resolver.py` — the render seam: `ResolvedCdVariant` + `resolve_cd_variant(db, storage, gremium_id)` and `cd_variant_key_for_gremium`.
- `service/cd_variants.py` — `CdVariantService`: variant CRUD, logo upload/vendored/move/reorder/delete, logo download bytes.
...
**Domain / data model:**
- `cd_variant` / `cd_variant_logo` — admin-managed corporate-design variants. A variant holds only a `key` (slug, unique, **immutable**), a display `name` and a `base_variant` (`report`/`protocol`, **create-only**). The PATCH changes the display name alone. `base_variant` stays out of it, because the render path pins the document shape per document kind. Its logos live in `cd_variant_logo`: one row per logo, in a `slot` (`title`/`footer`) at a `position`. A row is EITHER a vendored pytex name OR an uploaded MinIO object (`object_key`/`file_name`/`mime`/`size`) — a DB CHECK holds that exactly-one-of rule. Colors and fonts are NOT part of a variant.
...
- `GET|POST /api/admin/gremien/{id}/memberships`, `PATCH|DELETE /api/admin/gremium-memberships/{id}` — memberships (admin.gremien). The PATCH changes the role or the term of office; the member and the Gremium stay immutable.
- `GET /api/gremien` — auth-only gremien dropdown source (no admin right, `authed_router`).
- `GET|POST /api/admin/cd-variants`, `PATCH|DELETE /api/admin/cd-variants/{id}`, `POST /api/admin/cd-variants/{id}/logos` (multipart), `POST .../logos/vendored`, `PUT .../logos/order`, `PATCH|DELETE /api/admin/cd-variant-logos/{id}`, `GET /api/admin/cd-variant-logos/{id}/file` — CD variants (admin.cd_variants). The logo PATCH takes `slot` and `position` and moves one logo between the slots or inside its slot. It never touches the stored bytes, the file name or the vendored name. Both the source and the target slot come back densely numbered from 0. An omitted `position` appends at the end of the target slot, and keeps the current place while the slot stays.
- CD-variant conflict codes (409): `cd_variant_in_use_gremium` (a Gremium still points at the variant) and `cd_variant_in_use_protocol` (a protocol snapshotted the key) on the variant delete, `cd_variant_key_immutable` on a PATCH that sends a different `key`, `cd_logo_count_limit` (8 uploaded logos per variant) and `cd_logo_total_limit` (3 MB of uploaded logos per variant) on the logo upload.
...
- `GET|POST /api/admin/webhooks`, `PATCH|DELETE /api/admin/webhooks/{id}` (webhook.manage, list also flow.configure).
