...
- **`db/roles.sql` is a MANUAL prod step** — compose only runs `alembic upgrade head`. It does NOT create roles or revoke grants. Without it the platform runs, but with no role separation. Run it in two stages: steps 1–4 before the migrations, step 5 after. The script is idempotent.
