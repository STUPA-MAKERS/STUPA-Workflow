...
- `meeting_agenda_item`: TOP = assigned `application_id` (nullable, NULL means a free-text TOP that uses `title`), `body` (per-TOP Markdown → final protocol), `position` (ordered), `non_public` (redacted in public protocol PDF, numbering preserved). UNIQUE `(meeting_id, application_id)`.
...
- `POST .../votes` — create and open the decision vote of a TOP. An application TOP allows exactly one, a free-text TOP allows many. `DELETE .../votes/{vote_id}` — delete a vote.
...
- Application-TOP votes fail fast. The application must be in a `vote` state that matches `config.gremiumId` before the vote opens. Otherwise voters waste ballots on a vote that cannot close.
