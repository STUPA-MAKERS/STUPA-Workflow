...
- Bank reconcile (#fints): per-booker `get_/set_/delete_fints_credential`; `fints_sync` (returns `done` or `needs_tan` — **human** enters the TAN; 409 = bank locked, don't retry) + `fints_submit_tan`; `import_statement_file(account_id, file_path)` (CAMT.053/MT940, no TAN); staged rows via `list_statement_lines` → `confirm_statement_line(line_id, {budgetId|matchExpenseId})` / `ignore_statement_line`. Binary downloads (invoice PDF, xlsx exports) and applicant magic-link / OAuth browser routes are intentionally not exposed.
